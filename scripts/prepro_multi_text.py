...
def tokenize_and_reformat(para_json):
    ...
    for imgid, item in enumerate(para_json):
        ...
        tokenized_paragraph = [[tok.text for tok in spacy_en.tokenizer(x) if tok.text not in [' ', '  ']] for x in sentences]  
        ...
        image['sentences'] = [{}]*len(tokenized_paragraph)
        for i in range(len(tokenized_paragraph)):
            image['sentences'][i]['tokens'] = tokenized_paragraph[i]
            image['sentences'][i]['raw']    = raw_paragraph
            image['sentences'][i]['imgid']  = imgid
            image['sentences'][i]['sentid'] = id
            image['sentences'][i]['id']     = id
        images.append(image)
