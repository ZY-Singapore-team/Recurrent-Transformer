...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.input_label_h5 != 'none':
            ...
            self.topic = self.h5_label_file['topics'][:]
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        # sentence distribution target (s_max, )
        num_dist = np.zeros([seq_per_img], dtype='float32')
        if ncap < seq_per_img:
            # we need to subsample (with replacement)
            seq = np.zeros([seq_per_img, self.seq_length], dtype = 'int')
            top = np.zeros([seq_per_img, 768], dtype = 'float32')
            for q in range(ncap):
                # ixl = random.randint(ix1,ix2)
                seq[q, :] = self.label[ix1+q, :self.seq_length]
                top[q, :] = self.topic[ix1+q]
            num_dist[ncap-1:] = 1
        else:
            # ixl = random.randint(ix1, ix2 - seq_per_img + 1)
            seq = self.label[ix1: ix1 + seq_per_img, :self.seq_length]
            top = self.topic[ix1: ix1 + seq_per_img, :]
            num_dist[seq_per_img-1:] = 1

        return seq, top, num_dist
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            # fetch image
            tmp_fc, tmp_att, tmp_seq, tmp_top, tmp_num_dist, ix, it_pos_now, tmp_wrapped = sample
            if tmp_wrapped:
                wrapped = True
            ...
            topic_batch.append(tmp_top)
            num_dist_batch.append(tmp_num_dist)
        ...
        fc_batch, att_batch, label_batch, topic_batch, num_dist_batch, gts, infos = \
            zip(*sorted(zip(fc_batch, att_batch, label_batch, topic_batch, num_dist_batch, gts,infos), key=lambda x: 0, reverse=True))
        data = {}
        ...
        data['topics'] = np.vstack(topic_batch)
        data['gts_end'] = np.vstack(num_dist_batch)
        # generate mask
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'h5_label_file'):
            seq, top, num_dist = self.get_captions(ix, self.seq_per_img)
        else:
            seq = None
        return (fc_feat, att_feat, seq, top, num_dist, ix, it_pos_now, wrapped)
