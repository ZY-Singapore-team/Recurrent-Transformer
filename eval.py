...
pred_fn = os.path.join(opt.eval_results_dir, '.saved_pred_'+ opt.id + '_' + opt.split + '.pth')
result_fn = os.path.join(opt.eval_results_dir, opt.id + '_' + opt.split + '.json')
...
if opt.only_lang_eval == 1 or (not opt.force and os.path.isfile(pred_fn)): 
    ...
    lang_stats = eval_utils.language_eval('test', predictions, vars(opt), opt.split)
    print(lang_stats)
    os._exit(0)
