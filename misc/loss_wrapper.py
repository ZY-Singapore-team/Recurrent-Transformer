...
class LossWrapper(torch.nn.Module):
    ...
    def forward(self, fc_feats, att_feats, labels, masks, att_masks, topic_vec, gts, gt_indices,
                sc_flag, struc_flag):
        opt = self.opt
        ...
        if struc_flag:
            ...
            prediction, hidden_out = self.model(fc_feats, att_feats, labels, topic_vec, att_masks)
            loss = self.crit(prediction, labels[:,1:], masks[:,1:])
            out['decoder_output'] = hidden_out
        else:
            self.model.eval()
            with torch.no_grad():
